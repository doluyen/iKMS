//
//  CellContactDetailCostAccount.m
//  iKMS
//
...
@implementation CellContactDetailCostAccount
...
-(void)loadUIWithData:(ContactObject*)object{
    ...
    costAccount = object.CostAccountId;
...
-(void)sendRequestGetAccountChildrenAll{
    [[AFAppDotNetAPIClient sharedClient] GET:kPathGetAccountChildrenAll parameters:NULL success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        int idx = -1;
        int index = -1;
        if (![costAccount isKindOfClass:[NSNull class]]) {
            idx = [costAccount intValue];
        }
...
//        txt_name.text= [options objectAtIndex:selectedIndex];
    ...
    [ActionSheetStringPicker showPickerWithTitle:@"Select Cost Account" rows:options initialSelection:index doneBlock:done cancelBlock:cancel origin:self];
}
